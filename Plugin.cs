...
namespace SCP4666
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            config4666LevelRarities = Config.Bind("SCP-4666 Rarities", "Level Rarities", "ExperimentationLevel:0, AssuranceLevel:0, VowLevel:0, OffenseLevel:0, AdamanceLevel:0, MarchLevel:0, RendLevel:0, DineLevel:0, TitanLevel:0, ArtificeLevel:0, EmbrionLevel:0, Modded:0", "Rarities for each level. See default for formatting.");
            ...
            ModAssets = AssetBundle.LoadFromFile(Path.Combine(Path.GetDirectoryName(Info.Location), ""));
            if (ModAssets == null)
            {
                Logger.LogError($"Failed to load custom assets.");
                return;
            }
            LoggerInstance.LogDebug($"Got AssetBundle at: {Path.Combine(sAssemblyLocation, "")}");

            Item Knife = ModAssets.LoadAsset<Item>("");
            if (Knife == null) { LoggerInstance.LogError("Error: Couldnt get enchanted knife from assets"); return; }
            LoggerInstance.LogDebug($"Got enchanted knife prefab");
            ...
            {
                EnemyType SCP4666 = ModAssets.LoadAsset<EnemyType>("");
                ...
                TerminalNode YulemanTN = ModAssets.LoadAsset<TerminalNode>("");
                TerminalKeyword YulemanTK = ModAssets.LoadAsset<TerminalKeyword>("");
